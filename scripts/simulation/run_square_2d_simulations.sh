...
if [[ $# -lt 2 || $# -gt 6 ]]; then
    ...
    echo
    echo "Example:"
    echo "  $0 src/simulation results/simulations 2.22 2.32 0.0005 200"
    exit 1
fi
...
LOG_FILE="${RESULTS_DIR}/simulation_logs.txt"
...
while awk -v t="$T" -v end="$T_END" 'BEGIN { exit !(t <= end + 1e-12) }'; do
    ...
    for SIZE in "${SIZES[@]}"; do
        EXEC="${EXECUTABLES_DIR}/ising2d-${SIZE}"
        ...
        echo "Running L=$SIZE at T=$TEMP_FMT"
